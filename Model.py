...
def add_contact():
    phonebook.append(f'{name.get()}; {surname.get()}; {last_name.get()}; {phone.get()};\n')
    update_book()
...
def select_contact():
    while True:
        if len(select.curselection()) > 0:
            ...
            selected_string_list = phonebook[index_pick].strip(';').split('; ')
...
def change_contact():
    if len(select.curselection()) > 0:
        ...
        phonebook[index_change] = f'{name.get()}; {surname.get()}; {last_name.get()}; {phone.get()};\n'
        update_book()
    else:
        View.no_selection_error()
